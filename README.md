...
Переклад ведеться з англійської та японської версії гри. Якщо ви знайшли помилку або хочете допомогти, то робіть Pull Request.
...
Розпакувавши ці файли ми отримаємо дві теки: `item-msgbnd-dcx` та `menu-msgbnd-dcx`. В кожній з цих тек знаходяться файли з розширенням `.fmg`. Ці файли містять в собі текстові дані, які відображаються в грі, у форматі `.xml`. Їх теж треба розпакувати, щоб працювати з текстом.

Назви файлів в обох теках змінювати **не можна**, вони мають залишатися японською.
...
Власні назви та специфічні терміни мають перекладатися відповідно до контексту гри.  Щоб не запам'ятовувати всі терміни, створений словник, який буде оновлюватися з часом - `vocabulary.yaml`.

Словник зберігає терміни у відповідних категоріях. Сам словник ніяк не використовується в коді, він лише для зручності.
## Автоматизація

### Запакування файлів

Щоб спростити процес тестування, запаковування було автоматизовано. Для цього використовується скрипт `build.py`. Він автоматично запаковує файли з перекладом та копіює їх в потрібну теку гри. Для роботи скрипта потрібно вкахати шлях до програми WitchyBND та шлях до теки з грою (безпосередньо до теки мови, яку хочете замінити).
...
Щоб звірити прогрес перекладу, можна використовувати скрипт `check.py`. Він перевіряє, які рядки ще не перекладені.
